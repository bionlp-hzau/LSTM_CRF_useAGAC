...
class embed(nn.Module):
    def __init__(self, ls, cti_size, wti_size, hre = False):
        ...
        self.hre = hre # hierarchical recurrent encoding 分层递归编码
    ...
    class sae(nn.Module): # self-attentive encoder
        ...
        class attn_mh(nn.Module): # multi-head attention
            ...
            def forward(self, q, k, v, mask):
                ...
                contiguous():view只能用在连续的变量上，如果在view之前用了transpose，
                             permute等，需要用contiguous()来返回一个contiguous copy。
                """
                z = self.Wo(z)
                z = x + self.dropout(z)
                #z = self.norm(z) # residual connection and dropout
                return z

        class ffn(nn.Module): # position-wise feed-forward networks 前馈网络
            def __init__(self, dim):
                super().__init__()
                dim_ffn = 2048
            ...
            def forward(self, x):
                z = x + self.layers(x) # residual connection  残差连接
