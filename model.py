...
class rnn_crf(nn.Module):
    ...
    def decode(self, xc, xw, lens): # for inference (进行推断)
...
class rnn(nn.Module):
    ...
    def init_state(self, b): # initialize RNN states
        n = NUM_LAYERS * NUM_DIRS # 2*2
        h = HIDDEN_SIZE // NUM_DIRS  # 1000//2
        hs = zeros(n, b, h) # hidden state
        """
        hs = torch.zeros(4,0,500)
        => tensor([], size=(4, 0, 500))
        """
        if RNN_TYPE == "LSTM":
...
class crf(nn.Module):
    def __init__(self, num_tags):
        ...
        print(type(randn(num_tags, num_tags)))
